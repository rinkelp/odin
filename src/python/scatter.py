...
def simulate_shot(traj, num_molecules, detector, traj_weights=None,
    ...
    for i,num in enumerate(num_per_shapshot):
        ...
        if num > 0: # else, we can just skip...
            ...
            threads = []
            def multi_helper(fargs):
                """ a helper function that performs either CPU or GPU calcs """
                intensities[:] += xraysim.scatter(*fargs)
                return

            # run dat shit
            if num_cpu > 0:
                logger.debug('Running CPU scattering code (%d/%d)...' % (num_cpu, num))
                cpu_args = (num_cpu, qxyz, rxyz, atomic_numbers, "CPU")
                t_cpu = Thread(target=multi_helper, args=(cpu_args))
                t_cpu.start()
            ...
            if num_gpu > 0:
                ...
                t_gpu = Thread(target=multi_helper, args=(gpu_args))
                t_gpu.start()
