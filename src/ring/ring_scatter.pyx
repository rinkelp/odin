...
def simulate(input_file, n_rotations__, qarray, out_file=None, Nphi__=360 , qres__=0.01, wavelen__=0.7293):#, output_dir='.' ):   
    """
    Parameters
    ----------
    input_file : string
    ...
    if input_file.split('.')[-1]  == 'coor':
        ...
        coor_and_CM_2_hdf( input_file, out_file )
...
# e.g. [133,154,170] --> ['133','154','170'] --> '133 154 170'
    ...
    rs = new RingScatter(str( out_file ), Nphi__, n_rotations__, qres__, wavelen__, qstring )

    del rs


#def toRings( qs, ring_scatter_file ,  ):
